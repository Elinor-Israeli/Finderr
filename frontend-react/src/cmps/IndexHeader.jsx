  ...
  <div className="index-header main layout">
    <div className={`index-header-container main-layout ${isSticky ? 'sticky' : ''}`}>
      ...
      <div className="links">
        ...
        {/* Sign In Link or Buttons */}
        {!user && (
          <>
            <span className="sign-in-link" onClick={() => { onOpenModal(); setIsSignup(false); }}>
              Sign in
            </span>
            <button
              className="join-btn-index-header"
              onClick={() => { onOpenModal(); setIsSignup(true); }}
            >
              Join
            </button>
          </>
